...
class QbvUDPApp;
    ...
    private:
        struct QueuedMessage
        {
            ...
            EtherGPTP* self;
        public:
            QueuedMessage(const simtime_t& time, cMessage* msg, EtherGPTP* self)
                : time(time)
                , msg(msg)
                , self(self)
            {
        ...
        struct App_QueuedMessage
        {
            simtime_t time;
            cMessage* msg;
            QbvUDPApp* self;
        public:
            App_QueuedMessage(const simtime_t& time, cMessage* msg, QbvUDPApp* self)
                : time(time)
                , msg(msg)
                , self(self)
            {
            ...
            bool operator<(const App_QueuedMessage& rhs) const
            {
                /// because we want to have the element with the *smallest*
                /// timestamp to be the one with the highest priority, we
                /// swap > and <
        ...
        void scheduleAtHWtime(const simtime_t time, cMessage* msg , EtherGPTP* self);
    ...
    public:
        ...
        void setOffset_self(simtime_t value);
