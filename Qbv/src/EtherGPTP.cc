...
void EtherGPTP::masterPort(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        ...
        {
            sendSync();
            clockGptp->scheduleAtHWtime(clockGptp->getHWtime(tableGptp->getOffset())+syncInterval,msg,this,tableGptp->getOffset());
        }
...
void EtherGPTP::sendSync()
{
    ...
    {
        tableGptp->setOriginTimestamp(clockGptp->getHWtime(tableGptp->getOffset()));
    ...
    sentTimeSync_self = clockGptp->getHWtime(tableGptp->getOffset());
    ...
    clockGptp->scheduleAtHWtime(clockGptp->getHWtime(tableGptp->getOffset())+SimTime(FollowUpInterval),selfMsgFollowUp,this,tableGptp->getOffset());
...
void EtherGPTP::sendFollowUp()
{
    ...
    else if (nodeType == BRIDGE_NODE)
    {
        ...
        residenceTime=tableGptp->getRateRatio().dbl()*(clockGptp->getHWtime(tableGptp->getOffset())-sentTimeSync_self);
...
{
    receivedTimeResponder = clockGptp->getHWtime(tableGptp->getOffset()); // t2

    if (NULL == selfMsgDelayResp)
        selfMsgDelayResp = new cMessage("selfMsgPdelayResp");

    clockGptp->scheduleAtHWtime(clockGptp->getHWtime(tableGptp->getOffset())+(SimTime)PDelayRespInterval,selfMsgDelayResp,this,tableGptp->getOffset());
}
...
void EtherGPTP::sendPdelayResp()
{
    ...
    gptp->setSentTime(clockGptp->getHWtime(tableGptp->getOffset()));//t3的真实数据，本来应该放在follow里发送的，当然确实也发送了，但是实际计算的时候还是从这个帧里取的数
    ...
    clockGptp->scheduleAtHWtime(clockGptp->getHWtime(tableGptp->getOffset())+(SimTime)RespFollowInterval,selfMsgRespFollow,this,tableGptp->getOffset());
}
...
//            schedulePdelay = clockGptp->getHWtime(tableGptp->getOffset())+ pdelayInterval;
//            clockGptp->scheduleAtHWtime(schedulePdelay,msg,this,tableGptp->getOffset());
...
void EtherGPTP::sendPdelayReq()
{
    ...
    send(frame, "lowerLayerOut");
    transmittedTimeRequester = clockGptp->getHWtime(tableGptp->getOffset());//t1
}
...
void EtherGPTP::processSync(gPtp_Sync* gptp)
{
    ...
    receivedTimeSync = clockGptp->getHWtime(tableGptp->getOffset());
...
void EtherGPTP::processFollowUp(gPtp_FollowUp* gptp)
{
    ...
    SimTime receivedTimeSyncBeforeSync = clockGptp->getHWtime(tableGptp->getOffset());

    double bits_follow=(MAC_HEADER + FOLLOW_UP_PACKET_SIZE + CRC_CHECKSUM)*8;
    SimTime packetTransmissionTime_follow = (SimTime)bits_follow/1000000000;
    tableGptp->setOffset(TimeSync_orign + tableGptp->getCorrectionField()+packetTransmissionTime_follow+tableGptp->getPeerDelay() - clockGptp->getHWtime(0));

    vTimeDifferenceGMafterSync.record(clockGptp->getHWtime(tableGptp->getOffset())-simTime());
...
{
    receivedTimeRequester = clockGptp->getHWtime(tableGptp->getOffset()); //t4
